...
<div class="container-fluid">
        <div class="row search-upper-banner">
            <div class="col-lg-12">
                <div class="container">
                    <h3 style="margin-top: 20px;color: black; font-weight: 350;"> {{ filteredList.length }} results for {{ this.$route.params.searched }} </h3>
                </div>
            </div>
        </div>
        <div class="container">
...
computed: {
    filteredList() {
          ...
          const value = this.$route.params.searched.toLowerCase();
          return this.$store.state.products.filter(function(product) {return (product.title.toLowerCase().indexOf(value) > -1 );});}
},
methods:{
